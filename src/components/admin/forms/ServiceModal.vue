      ...
      <div
        ...
        <div
                    ...
                    <div class="grid grid-cols-1 gap-4">
                      ...
                      <div class="border-t pt-4 mt-4">
                        ...
                        <div class="flex items-center mb-3">
                          <input id="single" name="single" v-model="single" type="checkbox"
                            class="shrink-0 mt-0.5 border-gray-200 rounded text-blue-600 focus:ring-blue-500 dark:bg-neutral-800 dark:border-neutral-700" />
                          <label for="single" class="text-sm text-gray-500 ms-3 dark:text-neutral-400">
          ...
          <div class="flex items-center justify-end gap-x-2 border-t px-4 py-3 dark:border-neutral-700">
            <button type="button" @click="saveNewService" class="bg-blue-600 hover:bg-blue-700 inline-flex items-center gap-x-2 rounded-lg px-3 py-2 text-sm font-medium text-white">
              Guardar cambios
            </button>
          </div>
        </div>
...
import { addService } from '@/API/pushData.ts';
  ...
  methods: {
    ...
    async saveNewService() {
      ...
      if (!this.imageFiles || this.imageFiles.length === 0) {
        return errorToast('Error', 'Por favor seleccione al menos una imagen', 5000, true, false);
      }
      ...
      const data = {
        title: this.title,
        description: this.description,
        images: this.imageFiles.map((file, index) => ({
      ...
      const res = await addService(data, this.imageFiles);
